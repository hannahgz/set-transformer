...
def plot_weights_as_heatmap(weights, savefig_path=None):
    ...
    plt.xticks(ticks=range(0, weights.size(1)), labels=range(0, weights.size(1)), fontsize=5, ha="center")  # Tick every 8th input
    plt.yticks(ticks=range(weights.size(0)), labels=[f"{i}" for i in range(weights.size(0))], rotation=0, ha="center")
    if savefig_path:
