...
def find_activation_peaks(layer, neuron, input_examples_file=None, min_peak_height=0.05, 
    ...
    if set_type_filter:
        ...
        if set_type_filter not in neuron_activations[neuron]:
            ...
            raise ValueError(f"Set type '{set_type_filter}' not found. Available set types: {available_sets}")
        ...
        print(f"Analyzing {len(all_activations)} activations for neuron {neuron} in layer {layer} (set type: {set_type_filter})")
    else:
        ...
        for set_type in neuron_activations[neuron]:
            ...
            all_activations.extend(activations)
            activation_by_type[set_type] = activations
        print(f"Analyzing {len(all_activations)} total activations for neuron {neuron} in layer {layer} (all set types combined)")
    ...
    plt.title(f'Neuron {neuron} in Layer {layer}{title_suffix} - Activation Histogram with {len(peaks)} Peaks')
    plt.xlabel('Activation Value')
    plt.ylabel('Frequency')
    plt.grid(alpha=0.3)
            ...
            if set_type_filter in input_examples:
                for i, act_value in enumerate(activation_by_type[set_type_filter]):
                    if i < len(input_examples[set_type_filter]):
                        all_activations_with_examples.append((act_value, input_examples[set_type_filter][i]))
            else:
                print(f"Warning: No input examples found for set type '{set_type_filter}'")
                ...
                if cur_set_type in input_examples:
                    for i, act_value in enumerate(activation_by_type[cur_set_type]):
                        if i < len(input_examples[cur_set_type]):
                            all_activations_with_examples.append((act_value, input_examples[cur_set_type][i]))
